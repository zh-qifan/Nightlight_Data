...
for (b in bound_files) {
  ...
  if (as.integer(code) < 24) {
    next
  }
  
  bound = read_sf(file.path(b, paste0("conjunto_de_datos/",code,"m.shp")))
  ...
  bound_output_file = file.path("./outputs/", basename(b), "Manzana_level_nightlight.csv")
