...
for (b in bound_files) {
  ...
  mun_bound = read_sf(file.path(b, paste0("conjunto_de_datos/",code,"mun.shp")))
  if (as.integer(code) != 0) {
    r_bound = read_sf(file.path(b, paste0("conjunto_de_datos/",code,"ar.shp")))
    r_bound$CVE_LOC = 0
    bound = read_sf(file.path(b, paste0("conjunto_de_datos/",code,"a.shp")))
    bound = rbind(bound, r_bound)    
  }
  else {
    bound = read_sf(file.path(b, paste0("conjunto_de_datos/",code,"a.shp")))
  }
  ...
  for (f in files) {
    ...
    mun_bound = st_transform(mun_bound, crs = st_crs(rast))
    ...
    mun_bound[[column_name]] = NA
    n = nrow(mun_bound)
    for (i in 1:n) {
      test_bound = mun_bound[i,]
      x_min = st_bbox(test_bound)[1] - resolution
      y_min = st_bbox(test_bound)[2] - resolution
      x_max = st_bbox(test_bound)[3] + resolution
      y_max = st_bbox(test_bound)[4] + resolution
  ...
  mun_bound = mun_bound %>% as_tibble() %>% select(-geometry)
  
  bound_output_file = file.path("./outputs/", basename(b), "AGEB_level_nightlight.csv")
  write.csv(bound, bound_output_file)
  mun_bound_output_file = file.path("./outputs/", basename(b), "mun_level_nightlight.csv")
  write.csv(mun_bound, mun_bound_output_file)
}
